...
export const postTutupKasir = async (credentials) => {
    try {
        const response = await web.post('/setSetoran', credentials);
...
const TutupKasirModal = ({ isOpen, onClose, onSubmit }) => {
    const [username, setUsername] = useState('');
    const [password, setPassword] = useState('');
    const [setoran, setHarga] = useState('');
    ...
    const handleFormSubmit = async (e) => {
        ...
        setError('')
        try {
            const setoranNumeric = Number(setoran.replace(/\./g, ''));
            const responseData = await postTutupKasir({ username, password, setoran: setoranNumeric });
            onClose();
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg z-50 w-96">
                ...
                <hr className="my-2" />
                <div className='text-center'>
                    <h3 className="text-lg font-bold mb-4">Tutup Kasir</h3>
                </div>
                {error !== '' &&
                ...
                <form onSubmit={handleFormSubmit}>
                    ...
                    <div className="mb-4">
                        <label className="inline-block text-gray-700 text-sm mb-2 font-semibold" htmlFor="setoran">
                            Setoran
                        </label>
                        <input
                            type="text"
...
export default TutupKasirModal;
